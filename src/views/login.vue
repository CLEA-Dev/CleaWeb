      ...
      <div class="p-8 space-y-6">
        ...
        <form @submit.prevent="handleLogin" class="space-y-4">
          ...
          <button 
            type="submit" 
            :disabled="isLoading"
            class="w-full mt-4 bg-teal-800 hover:bg-teal-900 text-white font-medium py-2.5 px-4 rounded-lg text-sm flex items-center justify-center space-x-2 shadow-md transition-colors disabled:opacity-50"
          >
...
const handleLogin = () => {
  ...
  seConnecter(email.value, password.value)
    .then((data) => {
      console.log('Connexion réussie :', data)

      // Récupération du jeton renvoyé par l'API
      ...
      // Sauvegarde des infos utilisateur si présent
      if (data.user) {
        localStorage.setItem('user', JSON.stringify(data.user))
      }

      // 3. Redirection directe vers le tableau de bord
      router.push('/')
    })
    .catch((erreur) => {
      console.error('Erreur de connexion :', erreur)
      if (erreur.response && erreur.response.data && erreur.response.data.message) {
        errorMessage.value = erreur.response.data.message
      } else {
        errorMessage.value = 'Identifiants incorrects ou erreur de connexion.'
      }
    })
    .finally(() => {
